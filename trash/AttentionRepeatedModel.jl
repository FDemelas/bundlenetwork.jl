...
function create_features(lt::AttentionModelFactory, B::SoftBundle; auxiliary = 0)
	...
	ϕ, ϕγ = device(ϕ), device(ϕγ)

	return reshape(ϕ, (size(ϕ)..., 1)), reshape(ϕγ, (size(ϕγ)..., 1))
end
...
function create_features(lt::AttentionModelFactory, B::BatchedSoftBundle; auxiliary = 0)
	feat_t = zeros(Float32, size_features(B.lt), 1, length(B.idxComp))
	feat_theta = zeros(Float32, size_comp_features(B.lt), 1, length(B.idxComp))
...
function (m::AttentionModel)(xt, xγ, idx, comps)
	...
	μ, σ2 = dropdims.(Flux.MLUtils.chunk(h, 2, dims = 1),dims=2)
	...
	ϵt, ϵk, ϵq = device(randn(m.rng, Float32, size(μ))), device(randn(m.rng, Float32, size(μ))), device(randn(m.rng, Float32, size(μ)))

	# sample the hidden representation of t and then give it as input to the decoder to compute t
	t = m.decoder_t(m.sample_t ? μ + ϵt .* sigma : μ )
	# sample the hidden representation of the key and the query
	hki = m.decoder_γk(device(m.sample_γ ? μ .+ ϵk .* sigma : μ ))
	
	hqi = m.decoder_γq(device(m.sample_γ ? μ .+ ϵq .* sigma : μ ))
	...
	aq = device(size(hqi, 3) > 1 ? Flux.MLUtils.chunk(hqi, size(hqi, 3); dims = 2) : [dropdims(hqi,dims=2)])
...
function create_NN(
	...
	encoder = Chain(f_norm, LSTM(size_features(lt) + size_comp_features(lt) => 2 * h_representation))
	...
	model = AttentionModel(device(encoder), device(decoder_t), device(decoder_γk), device(decoder_γq), rng, sampling_t, sampling_θ, device(Zygote.bufferfrom(device(zeros(Float32, bs * h_representation, it)))), 1.0, h_representation, 1)
	return model
end
