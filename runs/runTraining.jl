...
function main(args)
	...
	res_folder =
		...
		string(h_representation) * "_" * string(sampling_θ) * string(sampling_t)* "_" * string(h_act) * "_" * string(use_softmax) * "_" * string(gamma) * "_" * string(lambda) * "_" * string(delta) * "_" * string(distribution_function) * "_" * string(bgr) * "_inc" * string(incremental)*"_rc"*string(reduced_components)*"_ss"*string(scheduling_ss)*"_h3"*string(h3)*"_rec"*string(recurrent)*"_rep"*string(repeated)*"_tanh"*string(use_tanh)
	...
	with_logger(lg) do
		for it in 1:maxEp
			...
			for it_idx in 1:ceil(mti / batch_size)
				...
				mv = 0.0
				B.maxIt = incremental ? min(2 * it * maxIt / maxEp, maxIt) : maxIt
				r_f = (batch_size > 1 || a_b ? sum( (contains(folder,"GA") ? f.inst.I : ( contains(folder,"TUC") ? f.inst.I : sizeE(f.inst))) for f in ϕ ) : contains(folder,"GA") ? ϕ.inst.I :  ( contains(folder,"TUC") ? ϕ.inst.I : sizeE(ϕ.inst)) ) * maxIt / (last - first + 1)
	...
	BundleNetworks.reset!(nn, 1)
	BundleNetworks.reset!(nn_best, 1)
